...
func TestParseInterfaceAndError(t *testing.T) {
	...
	expected, err := os.ReadFile("testdata/error/parser_expected.json")
	assert.NoError(t, err)
...
func TestParseTypeOverrides(t *testing.T) {
	...
	p := New(SetOverrides(map[string]string{
		"github.com/venosm/swaggo/testdata/global_override/types.Application":  "string",
		"github.com/venosm/swaggo/testdata/global_override/types.Application2": "github.com/venosm/swaggo/testdata/global_override/othertypes.Application",
		"github.com/venosm/swaggo/testdata/global_override/types.ShouldSkip":   "",
	}))
	err := p.ParseAPI(searchDir, mainAPIFile, defaultParseDepth)
	assert.NoError(t, err)
...
func TestParser_skipPackageByPrefix(t *testing.T) {
	...
	assert.False(t, parser.skipPackageByPrefix("github.com/venosm/swaggo"))
	assert.False(t, parser.skipPackageByPrefix("github.com/venosm/swaggo/cmd"))
	assert.False(t, parser.skipPackageByPrefix("github.com/venosm/swaggo/gen"))

	parser = New(SetPackagePrefix("github.com/venosm/swaggo/cmd"))

	assert.True(t, parser.skipPackageByPrefix("github.com/venosm/swaggo"))
	assert.False(t, parser.skipPackageByPrefix("github.com/venosm/swaggo/cmd"))
	assert.True(t, parser.skipPackageByPrefix("github.com/venosm/swaggo/gen"))

	parser = New(SetPackagePrefix("github.com/venosm/swaggo/cmd,github.com/venosm/swaggo/gen"))

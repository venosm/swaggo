...
func TestGen_BuildDescriptionWithQuotes(t *testing.T) {
	...
	cmd := exec.Command("go", "build", "-buildmode=plugin", "github.com/venosm/swaggo/testdata/quotes")
...
func TestGen_BuildDocCustomDelims(t *testing.T) {
	...
	cmd := exec.Command("go", "build", "-buildmode=plugin", "github.com/venosm/swaggo/testdata/delims")
...
func TestGen_FileDescriptionDirective(t *testing.T) {
	searchDir := "../testdata/file_description"
	config := &Config{
		SearchDir:          searchDir,
		MainAPIFile:        "./main.go",
		OutputDir:          searchDir + "/docs",
		OutputTypes:        outputTypes,
		PropNamingStrategy: "",
	}

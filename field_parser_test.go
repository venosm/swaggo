...
func TestDescriptionTag(t *testing.T) {
	t.Run("Description tag", func(t *testing.T) {
		t.Parallel()
		...
		err := newTagBaseFieldParser(
			...
			&ast.Field{
				...
				Tag: &ast.BasicLit{
					Value: `json:"test_field" description:"Test field description"`,
				},
			},
		).ComplementSchema(&schema)
		assert.NoError(t, err)
		assert.Equal(t, "Test field description", schema.Description)
	})

	t.Run("Description tag priority over comment", func(t *testing.T) {
		t.Parallel()
		...
		err := newTagBaseFieldParser(
			...
			&ast.Field{
				Names: []*ast.Ident{{Name: "TestField"}},
				Tag: &ast.BasicLit{
					Value: `json:"test_field" description:"Tag description"`,
				},
				Comment: &ast.CommentGroup{
					List: []*ast.Comment{{Text: "// Comment description"}},
				},
			},
		).ComplementSchema(&schema)
		assert.NoError(t, err)
		assert.Equal(t, "Tag description", schema.Description)
	})
}
